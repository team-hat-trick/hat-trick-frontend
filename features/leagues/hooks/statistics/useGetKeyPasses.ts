...
export const useGetKeyPasses = (leagueId: number, season: number) => {
  const supabase = createBrowserSupabaseClient();

  return useQuery({
    queryKey: ["key-passes", leagueId, season],
    ...
    queryFn: async () => {
      let { data, error } = await supabase
        ...
        .eq("season", season)
        .order("key_passes", { ascending: false })
        .limit(5);
      ...
      if (!data || data.length === 0) {
        ...
        if (!syncRes.ok) throw new Error("키 패스 데이터 동기화 실패");
