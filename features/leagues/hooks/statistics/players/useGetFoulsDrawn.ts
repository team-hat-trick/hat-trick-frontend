...
export const useGetFoulsDrawn = (leagueId: number, season: number) => {
    const supabase = createBrowserSupabaseClient();

    return useQuery({
        queryKey: ["fouls-drawn", leagueId, season],
        ...
        queryFn: async () => {
            let { data, error } = await supabase
                ...
                .eq("season", season)
                .order("fouls_drawn", { ascending: false })
                .limit(5);
            ...
            if (!data || data.length === 0) {
                ...
                if (!syncRes.ok) throw new Error("파울 유도 데이터 동기화 실패");
