...
export const useGetTackleSuccess = (leagueId: number, season: number) => {
    const supabase = createBrowserSupabaseClient();

    return useQuery({
        queryKey: ['tackle-success', leagueId, season],
        ...
        queryFn: async () => {
            let { data, error } = await supabase
                ...
                .eq("season", season)
                .order("tackle_success", { ascending: false })
                .limit(5);
            ...
            if (!data || data.length === 0) {
                ...
                if (!syncRes.ok) throw new Error("태클 성공률 데이터 동기화 실패");
