...
export const useGetPenaltySuccessRate = (leagueId: number, season: number) => {
    const supabase = createBrowserSupabaseClient();

    return useQuery({
        queryKey: ['penalty-success-rate', leagueId, season],
        ...
        queryFn: async () => {
            let { data, error } = await supabase
                ...
                .eq("season", season)
                .order("penalty_success_rate", { ascending: false })
                .limit(5);
            ...
            if (!data || data.length === 0) {
                ...
                if (!syncRes.ok) throw new Error("페널티킥 성공률 데이터 동기화 실패");
