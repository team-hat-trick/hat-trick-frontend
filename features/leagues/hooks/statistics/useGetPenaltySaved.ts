...
export const useGetPenaltySaved = (leagueId: number, season: number) => {
  const supabase = createBrowserSupabaseClient();

  return useQuery({
    queryKey: ["penalty-saved", leagueId, season],
    ...
    queryFn: async () => {
      let { data, error } = await supabase
        ...
        .eq("season", season)
        .not("penalty_saved", "is", null)
        .order("penalty_saved", { ascending: false })
        .limit(5);
      ...
      if (!data || data.length === 0) {
        ...
        if (!syncRes.ok) throw new Error("페널티킥 선방 데이터 동기화 실패");
